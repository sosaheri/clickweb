...
class FinanceController extends Controller
{
    ...
    public function ownerFinances()
    {
        ...
        //CARDS
        $cards = [
            ['title'=>'Numero pedidos totales', 'value'=>0],
            ['title'=>'Total', 'value'=>0, 'isMoney'=>true],
            ...
            ['title'=>'Total del Ultimo mes', 'value'=>0, 'isMoney'=>true],
              ['title'=>'Total del Ultimo año', 'value'=>0, 'isMoney'=>true],
        ];
        foreach ($resources['orders']->get() as $key => $order) {
            $cards[0]['value'] += 1;
            $cards[1]['value'] += $order->delivery_price + $order->order_price;
        ...
        $ordersMon=Order::where('restorant_id', '=', $restaurant->id)->where(
    'created_at', '>=', Carbon::today()->startOfMonth()->subMonth()
)->get();
       $ordersYea=Order::where('restorant_id', '=', $restaurant->id)->whereYear('created_at', date('Y', strtotime('-1 year')))->get();
             foreach ($ordersMon as $key => $order) {
         
            $cards[8]['value'] += $order->delivery_price + $order->order_price;
          
        }
                foreach ($ordersYea as $key => $orderl) {
         
         
             $cards[9]['value'] += $orderl->delivery_price + $orderl->order_price;
        }
