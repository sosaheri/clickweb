...
class FrontEndController extends Controller
{
    ...
    public function whatsappMode()
    {
        ...
        public function showServices($city_alias)
    {
        //Variants
        /**
        ...
        if ($hasCity) {
            ...
            array_push($sections, ['title'=>__('Restaurants in').' ', 'restorants' =>$restorants]);
        } elseif ($hasLocation) {
    ...
    public function showService()
    {
        ...
        $sections = [];
          $theRestorants = Restorant::where(['active'=>1, 'category'=>3])->get();
             array_push($sections, ['title'=>__('Lista de Servicios').' ', 'restorants' =>$theRestorants]);
        ...
        foreach ($banners_data as $key => $banner) {
            if ((new Carbon($banner->active_to))->gt(new Carbon($banner->active_from)) && Carbon::now()->between(new Carbon($banner->active_from), new Carbon($banner->active_to)) || (new Carbon($banner->active_from))->eq(new Carbon($banner->active_to)) && (new Carbon(Carbon::now()->toDateString()))->eq(new Carbon($banner->active_from)) && (new Carbon(Carbon::now()->toDateString()))->eq(new Carbon($banner->active_to))) {
              ...
              //Set the cookie of the last entered address
        $lastaddress = Cookie::get('lastaddress');
        $response = new \Illuminate\Http\Response(view('service', [
            'sections' => $sections,
            'lastaddress'=> $lastaddress,
            'banners' => $banners,
        ]));
    ...
    public function showShops()
    {
        ...
        $sections = [];
          $theRestorants = Restorant::where(['active'=>1, 'category'=>2])->get();
             array_push($sections, ['title'=>__('Lista de Tiendas').' ', 'restorants' =>$theRestorants]);
...
public function destroy($id){
        ...
        if (auth()->user()->hasRole('client')) {
               ...
               Lists::where(['restaurant_id' => $id, 'user_id'=>auth()->user()->id])->delete();
               return redirect()->route('show.link');
...
public function addList($id){
        ...
        if (auth()->user()->hasRole('client')) {
               ...
               Lists::firstOrCreate(['restaurant_id' => $id, 'user_id'=>auth()->user()->id]);
                return redirect()->route('show.link');
              ...
              public function showLink()
    {
        ...
        $sections = [];
          ...
          $lists  =$lists->where(['user_id'=>auth()->user()->id]);
           $lists = $lists->with([ 'restorant'])->get()->toArray();
                    Log::warning(  $lists );
                          $items = [];
        ...
        $banners = [];
            ...
            array_push($items,$list['restorant'] );
        }
           Log::warning(  'wow');
              Log::warning(  $items );

             array_push($sections, ['title'=>__('Lista de Links').' ', 'restorants' =>$items]);
        foreach ($banners_data as $key => $banner) {
            if ((new Carbon($banner->active_to))->gt(new Carbon($banner->active_from)) && Carbon::now()->between(new Carbon($banner->active_from), new Carbon($banner->active_to)) || (new Carbon($banner->active_from))->eq(new Carbon($banner->active_to)) && (new Carbon(Carbon::now()->toDateString()))->eq(new Carbon($banner->active_from)) && (new Carbon(Carbon::now()->toDateString()))->eq(new Carbon($banner->active_to))) {
                array_push($banners, $banner);
            }
              ...
              //Set the cookie of the last entered address
        $lastaddress = Cookie::get('lastaddress');
        $response = new \Illuminate\Http\Response(view('links', [
            'sections' => $sections,
            'lastaddress'=> $lastaddress,
            'banners' => $banners,
        ])->with('res', $items));
    ...
    public function showStores($city_alias)
    {
        ...
        if ($hasCity) {
            ...
            array_push($sections, ['title'=>__('Lista de Links en').' '.$city->name.$aditionInTitle, 'restorants' =>$restorants]);
        } elseif ($hasLocation) {
            //LOCATION BASED SEARCH CASE 4 and 5
            //First, find the provided location, convert it to lat/lng
            $client = new \GuzzleHttp\Client();
