...
@if(auth()->user()->hasRole('admin') || auth()->user()->hasRole('owner') || auth()->user()->hasRole('driver'))
    ...
    <td>
        ...
        @elseif($lastStatusAlisas == "assigned_to_driver")
             <a href="{{ url('updatestatus/accepted_by_driver/'.$order->id) }}" class="btn btn-success btn-sm order-action">{{ __('Accept') }}</a>
             <a href="{{ url('updatestatus/rejected_by_driver/'.$order->id) }}" class="btn btn-danger btn-sm order-action">{{ __('Reject') }}</a>
               @elseif($lastStatusAlisas == "accepted_by_driver")
              <a href="{{'updatestatus/picked_up/'.$order->id }}" class="btn btn-primary btn-sm order-action">{{ __('Picked Up') }}</a>
